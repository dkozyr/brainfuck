...
std::vector<Operand> Optimizer::Process(const std::vector<Operand>& program) {
...
std::vector<Operand> Optimizer::ProcessWhile(const std::vector<Operand>& program, size_t& idx, int32_t offset) {
    const auto size = program.size();
...
void Optimizer::PushArithmeticOperands(std::unordered_map<int32_t, int32_t>& offset_to_delta, std::vector<Operand>& optimized) {
    for(auto& value: offset_to_delta) {
        if(value.second > 0) {
...
void Optimizer::PushDeltaOperand(const int32_t offset, const int32_t offset_prev, std::vector<Operand>& optimized) {
